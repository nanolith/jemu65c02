...
/**
 * 0xAC LDY abs normal result (not negative or zero).
 */
TEST(step_LDY_abs_basics)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    ...
    /* at 0x2000, add the LDY input. */
    mem[0x2000] = EXPECTED_LDY_INPUT;
...
/**
 * 0xAC LDY abs Zero result.
 */
TEST(step_LDY_abs_zero)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0xAC LDY abs, the negative flag is set.
 */
TEST(step_LDY_abs_negative)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
