...
/**
 * 0x56 LSR zer x Normal result (not zero; no carry).
 */
TEST(step_LSR_zer_x_basics)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    ...
    /* at 0x000A, add the LSR input. */
    mem[0x000A] = EXPECTED_LSR_INPUT;
    ...
    /* POSTCONDITION: mem[0x000A] is set to the expected output. */
    TEST_EXPECT(EXPECTED_LSR_OUTPUT == mem[0x000A]);
...
/**
 * 0x56 LSR zer x high bit set to zero. (not zero; no carry).
 */
TEST(step_LSR_zer_x_high_bit)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0x56 LSR zer a shift low bit to carry, not zero.
 */
TEST(step_LSR_zer_a_low_bit_carry)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0x56 LSR zer x shift low bit to carry, zero.
 */
TEST(step_LSR_zer_x_low_bit_carry_zero)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0x56 LSR zer x shift zero is zero.
 */
TEST(step_LSR_zer_x_zero)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
