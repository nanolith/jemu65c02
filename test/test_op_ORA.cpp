...
/**
 * ORA normal (non-zero / non-negative).
 */
TEST(op_ORA_basics)
{
    j65c02* inst = nullptr;
    ...
    JEMU_SYM(j65c02_op_ORA)(inst, EXPECTED_ORA_INPUT);

    /* POSTCONDITION: crash flag is not set. */
    TEST_EXPECT(!j65c02_crash_flag_get(inst));

    /* POSTCONDITION: A is set to the output value. */
