...
/**
 * 0xBE LDX abs y normal result (not negative or zero).
 */
TEST(step_LDX_abs_y_basics)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    const uint8_t EXPECTED_LDX_INPUT = 0x7F;
    ...
    /* at 0x1000, add an LDX instruction. */
    mem[0x1000] = 0xBE;
    mem[0x1001] = 0x00;
    mem[0x1002] = 0x20;

    /* at 0x2005, add the LDX input. */
    mem[0x2005] = EXPECTED_LDX_INPUT;
    ...
    /* PRECONDITION: X is set to 0x00. */
    j65c02_reg_x_set(inst, 0x00);
    ...
    /* POSTCONDITION: X is set to the expected input. */
    TEST_EXPECT(j65c02_reg_x_get(inst) == EXPECTED_LDX_INPUT);
...
/**
 * 0xBE LDA abs y Zero result.
 */
TEST(step_LDX_abs_y_zero)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    const uint8_t EXPECTED_LDX_INPUT = 0x00;
    ...
    /* PRECONDITION: X is set to 0xFF. */
    j65c02_reg_x_set(inst, 0xFF);
...
/**
 * 0xBE LDX abs y, the negative flag is set.
 */
TEST(step_LDX_abs_y_negative)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    const uint8_t EXPECTED_LDX_INPUT = 0x80;
