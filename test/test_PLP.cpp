...
/**
 * 0x28 PLP instruction.
 */
TEST(step_PLP)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    const uint8_t LOADED_VAL = 0xFF;
    const uint8_t EXPECTED_VAL = LOADED_VAL & ~JEMU_65c02_STATUS_NC;
    ...
    /* at 0x1000, add a PLP instruction. */
    mem[0x1000] = 0x28;

    /* at 0x0150, set our loaded value. */
    mem[0x0150] = LOADED_VAL;
    ...
    /* PRECONDITION: all flags are clear. */
    j65c02_reg_status_set(inst, 0x00);
    ...
    /* POSTCONDITION: All flags EXCEPT NC are set. */
    TEST_EXPECT(EXPECTED_VAL == j65c02_reg_status_get(inst));
