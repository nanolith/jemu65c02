...
/**
 * 0x55 EOR zer x normal result (not negative or zero).
 */
TEST(step_EOR_zer_x_basics)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    ...
    /* at 0x000A, add the EOR input. */
    mem[0x000A] = EXPECTED_EOR_INPUT;
...
/**
 * 0x55 EOR zer x Zero result.
 */
TEST(step_EOR_zer_x_zero)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0x55 EOR zer x, the negative flag is set.
 */
TEST(step_EOR_zer_x_negative)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
