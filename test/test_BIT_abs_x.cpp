...
TEST(step_BIT_abs_x_zero)
{
    ...
    /* at 0x1000, add a BIT abs x instruction. */
    mem[0x1000] = 0x3C;
    mem[0x1001] = 0x00;
    mem[0x1002] = 0x20;

    /* at 0x2005, set the value. */
    mem[0x2005] = 0x20;
...
/**
 * 0x3C BIT abs x not zero.
 */
TEST(step_BIT_abs_x_not_zero)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    const uint8_t EXPECTED_A = 0x10;
...
/**
 * 0x3C BIT abs x negative.
 */
TEST(step_BIT_abs_x_negative)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    const uint8_t EXPECTED_A = 0x10;
...
/**
 * 0x2C BIT abs x overflow.
 */
TEST(step_BIT_abs_x_overflow)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    const uint8_t EXPECTED_A = 0x10;
