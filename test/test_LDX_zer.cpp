...
/**
 * 0xA6 LDX zer normal result (not negative or zero).
 */
TEST(step_LDX_zer_basics)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    ...
    /* at 0x0005, add the LDX input. */
    mem[0x0005] = EXPECTED_LDX_INPUT;
...
/**
 * 0xA6 LDX zer Zero result.
 */
TEST(step_LDX_zer_zero)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0xA6 LDX zer, the negative flag is set.
 */
TEST(step_LDX_zer_negative)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
