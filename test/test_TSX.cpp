...
/**
 * 0xBA TSX instruction.
 */
TEST(step_TSX)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    const uint8_t EXPECTED_S_VALUE = 0x12;
    ...
    j65c02_reg_sp_set(inst, EXPECTED_S_VALUE);

    /* PRECONDITION: set X to 0. */
    j65c02_reg_x_set(inst, 0);
    ...
    /* POSTCONDITION: X is equal to S. */
    TEST_EXPECT(EXPECTED_S_VALUE == j65c02_reg_x_get(inst));
...
/**
 * 0xBA TSX instruction negative result.
 */
TEST(step_TSX_negative)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    const uint8_t EXPECTED_S_VALUE = 0x82;
    ...
    /* PRECONDITION: set S to the expected S value. */
    j65c02_reg_sp_set(inst, EXPECTED_S_VALUE);
...
/**
 * 0xBA TSX instruction zero flag is set.
 */
TEST(step_TSX_zero)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    const uint8_t EXPECTED_S_VALUE = 0x00;
