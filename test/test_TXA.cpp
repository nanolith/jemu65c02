...
TEST(step_TXA)
{
    ...
    /* PRECONDITION: set X to the expected A value. */
    j65c02_reg_x_set(inst, EXPECTED_A_VALUE);
    ...
    /* POSTCONDITION: A is equal to X. */
    TEST_EXPECT(EXPECTED_A_VALUE == j65c02_reg_a_get(inst));
...
/**
 * 0x8A TXA instruction negative result.
 */
TEST(step_TXA_negative)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    ...
    /* POSTCONDITION: A is equal to X. */
    TEST_EXPECT(EXPECTED_A_VALUE == j65c02_reg_x_get(inst));
...
/**
 * 0x8A TXA instruction zero flag is set.
 */
TEST(step_TXA_zero)
{
    j65c02* inst = nullptr;
