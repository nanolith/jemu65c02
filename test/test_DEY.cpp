...
/**
 * 0x88 DEY (non-zero / non-negative to non-zero / non-negative).
 */
TEST(step_DEY_non_zero_non_negative)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    ...
    /* at 0x1000, add a DEY instruction. */
    mem[0x1000] = 0x88;
    ...
    /* POSTCONDITION: Y is 0x01. */
    TEST_EXPECT(0x01 == j65c02_reg_y_get(inst));
...
/**
 * 0x88 DEY (0x00 to 0xFF).
 */
TEST(step_DEY_negative)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    ...
    /* POSTCONDITION: Y is 0xFF. */
    TEST_EXPECT(0xFF == j65c02_reg_y_get(inst));
