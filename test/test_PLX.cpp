...
TEST(step_PLX)
{
    ...
    /* at 0x1000, add a PLX instruction. */
    mem[0x1000] = 0xFA;
    ...
    /* POSTCONDITION: X is set to our expected value. */
    TEST_EXPECT(EXPECTED_VAL == j65c02_reg_x_get(inst));
...
/**
 * 0xFA PLX if the pulled value is zero, the zero flag is set.
 */
TEST(step_PLX_zero)
{
    j65c02* inst = nullptr;
...
/**
 * 0xFA PLX if the pulled value is negative, the negative flag is set.
 */
TEST(step_PLX_negative)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
