...
/**
 * 0xB4 LDY zer x normal result (not negative or zero).
 */
TEST(step_LDY_zer_x_basics)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    ...
    /* at 0x000A, add the LDY input. */
    mem[0x000A] = EXPECTED_LDY_INPUT;
...
/**
 * 0xB4 LDY zer x Zero result.
 */
TEST(step_LDY_zer_x_zero)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0xB4 LDY zer x, the negative flag is set.
 */
TEST(step_LDY_zer_x_negative)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
