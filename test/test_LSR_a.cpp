...
/**
 * 0x4A LSR a Normal result (not zero; no carry).
 */
TEST(step_LSR_a_basics)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    ...
    /* PRECONDITION: set A to the expected input. */
    j65c02_reg_a_set(inst, EXPECTED_LSR_INPUT);
    ...
    /* POSTCONDITION: PC is set to 0x1001. */
    TEST_EXPECT(0x1001 == j65c02_reg_pc_get(inst));

    /* POSTCONDITION: A is set to the expected output. */
    TEST_EXPECT(EXPECTED_LSR_OUTPUT == j65c02_reg_a_get(inst));
...
/**
 * 0x4A LSR a high bit set to zero. (not zero; no carry).
 */
TEST(step_LSR_a_high_bit)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0x4A LSR a shift low bit to carry, not zero.
 */
TEST(step_LSR_a_low_bit_carry)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0x4A LSR a shift low bit to carry, zero.
 */
TEST(step_LSR_a_low_bit_carry_zero)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0x4A LSR a shift zero is zero.
 */
TEST(step_LSR_a_zero)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
