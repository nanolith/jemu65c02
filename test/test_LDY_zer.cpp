...
/**
 * 0xA4 LDY zer normal result (not negative or zero).
 */
TEST(step_LDY_zer_basics)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    ...
    /* at 0x0005, add the LDY input. */
    mem[0x0005] = EXPECTED_LDY_INPUT;
...
/**
 * 0xA4 LDY zer Zero result.
 */
TEST(step_LDY_zer_zero)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0xA4 LDY zer, the negative flag is set.
 */
TEST(step_LDY_zer_negative)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
