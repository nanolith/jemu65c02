...
/**
 * 0xB6 LDX zer y normal result (not negative or zero).
 */
TEST(step_LDX_zer_y_basics)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    ...
    /* at 0x000A, add the LDX input. */
    mem[0x000A] = EXPECTED_LDX_INPUT;
...
/**
 * 0xB6 LDX zer y Zero result.
 */
TEST(step_LDX_zer_y_zero)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0xB6 LDX zer y, the negative flag is set.
 */
TEST(step_LDX_zer_y_negative)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
