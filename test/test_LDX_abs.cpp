...
/**
 * 0xAE LDX abs normal result (not negative or zero).
 */
TEST(step_LDX_abs_basics)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    ...
    /* at 0x2000, add the LDX input. */
    mem[0x2000] = EXPECTED_LDX_INPUT;
...
/**
 * 0xAE LDX abs Zero result.
 */
TEST(step_LDX_abs_zero)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0xAE LDX abs, the negative flag is set.
 */
TEST(step_LDX_abs_negative)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
