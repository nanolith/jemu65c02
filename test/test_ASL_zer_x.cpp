...
TEST(step_ASL_zer_x)
{
    ...
    /* at 0x1000, add an ASL zer instruction. */
    mem[0x1000] = 0x16;
    mem[0x1001] = 0x05;

    /* at 0x000A, set the input value. */
    ...
    TEST_EXPECT(mem[0x000A] == EXPECTED_OUTPUT);
...
/**
 * 0x16 ASL ZER X sets the carry bit if the high bit is shifted out.
 */
TEST(step_ASL_zer_x_carry_flag)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    ...
    /* PRECONDITION: clear the carry bit. */
    j65c02_reg_status_set(
        inst, j65c02_reg_status_get(inst) & ~JEMU_65c02_STATUS_CARRY);
...
/**
 * 0x16 ASL ZER X sets the negative bit if the high bit is set after shifting.
 */
TEST(step_ASL_zer_x_negative_flag)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0x16 ASL ZER X sets the zero flag if the result is zero.
 */
TEST(step_ASL_zer_x_zero_flag)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
