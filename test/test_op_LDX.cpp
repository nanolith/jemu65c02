...
/**
 * LDX Normal result (not negative or zero).
 */
TEST(LDX_basics)
{
    j65c02* inst = nullptr;
    ...
    /* run the LDX operation. */
    JEMU_SYM(j65c02_op_LDX)(inst, 0x10);

    /* POSTCONDITION: crash flag is not set. */
    TEST_EXPECT(!j65c02_crash_flag_get(inst));

    /* POSTCONDITION: X is set to 0x10. */
    TEST_EXPECT(0x10 == j65c02_reg_x_get(inst));
...
TEST(LDX_zero)
{
    ...
    /* run the LDX operation. */
    JEMU_SYM(j65c02_op_LDX)(inst, 0x00);

    /* POSTCONDITION: crash flag is not set. */
    TEST_EXPECT(!j65c02_crash_flag_get(inst));

    /* POSTCONDITION: X is set to 0x00. */
    TEST_EXPECT(0x00 == j65c02_reg_x_get(inst));
...
TEST(LDX_negative)
{
    ...
    /* run the LDX operation. */
    JEMU_SYM(j65c02_op_LDX)(inst, 0x80);

    /* POSTCONDITION: crash flag is not set. */
    TEST_EXPECT(!j65c02_crash_flag_get(inst));

    /* POSTCONDITION: X is set to 0x80. */
    TEST_EXPECT(0x80 == j65c02_reg_x_get(inst));
