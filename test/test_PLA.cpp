...
TEST(step_PLA)
{
    ...
    /* at 0x1000, add a PLA instruction. */
    mem[0x1000] = 0x68;
    ...
    /* POSTCONDITION: A is set to our expected value. */
    TEST_EXPECT(EXPECTED_VAL == j65c02_reg_a_get(inst));
...
/**
 * 0x68 PLA if the pulled value is zero, the zero flag is set.
 */
TEST(step_PLA_zero)
{
    j65c02* inst = nullptr;
...
/**
 * 0x68 PLA if the pulled value is negative, the negative flag is set.
 */
TEST(step_PLA_negative)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
