...
/**
 * LDA Normal result (not negative or zero).
 */
TEST(LDA_basics)
{
    j65c02* inst = nullptr;
    ...
    /* run the LDA operation. */
    JEMU_SYM(j65c02_op_LDA)(inst, 0x10);

    /* POSTCONDITION: crash flag is not set. */
    TEST_EXPECT(!j65c02_crash_flag_get(inst));

    /* POSTCONDITION: A is set to 0x10. */
    TEST_EXPECT(0x10 == j65c02_reg_a_get(inst));
...
TEST(LDA_zero)
{
    ...
    /* run the LDA operation. */
    JEMU_SYM(j65c02_op_LDA)(inst, 0x00);

    /* POSTCONDITION: crash flag is not set. */
    TEST_EXPECT(!j65c02_crash_flag_get(inst));

    /* POSTCONDITION: A is set to 0x00. */
...
TEST(LDA_negative)
{
    ...
    /* run the LDA operation. */
    JEMU_SYM(j65c02_op_LDA)(inst, 0x80);

    /* POSTCONDITION: crash flag is not set. */
    TEST_EXPECT(!j65c02_crash_flag_get(inst));

    /* POSTCONDITION: A is set to 0x80. */
