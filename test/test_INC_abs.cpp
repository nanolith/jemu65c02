...
/**
 * 0xEE INC abs (non-zero / non-negative to non-zero / non-negative).
 */
TEST(step_INC_abs_non_zero_non_negative)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    ...
    /* POSTCONDITION: mem[0x2000] is 0x02. */
    TEST_EXPECT(0x02 == mem[0x2000]);
...
/**
 * 0xEE INC abs (0xFF to zero).
 */
TEST(step_INC_abs_zero)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0xEE INC abs (0x7F to 0x80).
 */
TEST(step_INC_abs_negative)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    ...
    /* POSTCONDITION: mem[0x2000] is 0x80 */
    TEST_EXPECT(0x80 == mem[0x2000]);
