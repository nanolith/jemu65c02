...
/**
 * 0x5E LSR abs x Normal result (not zero; no carry).
 */
TEST(step_LSR_abs_x_basics)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
    ...
    /* at 0x2005, add the LSR input. */
    mem[0x2005] = EXPECTED_LSR_INPUT;
    ...
    /* POSTCONDITION: mem[0x2005] is set to the expected output. */
    TEST_EXPECT(EXPECTED_LSR_OUTPUT == mem[0x2005]);
...
/**
 * 0x5E LSR abs x high bit set to zero. (not zero; no carry).
 */
TEST(step_LSR_abs_x_high_bit)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0x5E LSR abs x shift low bit to carry, not zero.
 */
TEST(step_LSR_abs_x_low_bit_carry)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0x4E LSR abs x shift low bit to carry, zero.
 */
TEST(step_LSR_abs_x_low_bit_carry_zero)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
...
/**
 * 0x5E LSR abs x shift zero is zero.
 */
TEST(step_LSR_abs_x_zero)
{
    j65c02* inst = nullptr;
    uint8_t mem[65536];
