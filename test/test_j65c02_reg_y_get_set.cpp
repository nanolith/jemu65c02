...
TEST_SUITE(j65c02_reg_y_get_set);
...
/**
 * Verify that we can set and get the Y register.
 */
TEST(get_set_basics)
{
    j65c02* inst = nullptr;
    ...
    /* PRECONDITION: Y is NOT the expected value. */
    TEST_ASSERT(EXPECTED_VALUE != j65c02_reg_y_get(inst));

    /* Set Y to the expected value. */
    j65c02_reg_y_set(inst, EXPECTED_VALUE);

    /* POSTCONDITION: Y is the expected value. */
    TEST_EXPECT(EXPECTED_VALUE == j65c02_reg_y_get(inst));
