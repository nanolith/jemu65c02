...
/**
 * LDY Normal result (not negative or zero).
 */
TEST(LDY_basics)
{
    j65c02* inst = nullptr;
    ...
    /* run the LDY operation. */
    JEMU_SYM(j65c02_op_LDY)(inst, 0x10);

    /* POSTCONDITION: crash flag is not set. */
    TEST_EXPECT(!j65c02_crash_flag_get(inst));

    /* POSTCONDITION: Y is set to 0x10. */
    TEST_EXPECT(0x10 == j65c02_reg_y_get(inst));
...
TEST(LDY_zero)
{
    ...
    /* run the LDY operation. */
    JEMU_SYM(j65c02_op_LDY)(inst, 0x00);

    /* POSTCONDITION: crash flag is not set. */
    TEST_EXPECT(!j65c02_crash_flag_get(inst));

    /* POSTCONDITION: Y is set to 0x00. */
...
TEST(LDY_negative)
{
    ...
    /* run the LDY operation. */
    JEMU_SYM(j65c02_op_LDY)(inst, 0x80);

    /* POSTCONDITION: crash flag is not set. */
    TEST_EXPECT(!j65c02_crash_flag_get(inst));

    /* POSTCONDITION: Y is set to 0x80. */
